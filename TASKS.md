...
|---|---|---|
| **Mobile header still likely wraps to two lines — FURTHER REDUCED 2026-08-24** | Theme toggle collapsed 3→1 (`028240826`), locale switch collapsed 2→1 — one link to the OTHER locale, not a pair (`029240826`). Both verified with real clicks in a browser (not `.click()` — see `learn.md`) and axe (0 violations, both directions, both tasks). **Minimum content width measured directly via DOM: ≈494px on `/en`, ≈482px on `/ar`** — down from ≈562px before tonight. A real phone is 360–430px, so the gap is now ≈50–135px depending on device, versus ≈130–200px before. Likely still wraps on most phones, closer on the wider ones. No more header controls left to remove without cutting nav items or menu-izing them on mobile — that's the next lever if this still isn't enough. **Needs a real phone check** — not re-tested on an actual device since either fix | Moataz |
