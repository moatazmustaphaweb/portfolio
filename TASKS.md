...
|---|---|---|
| ~~Mobile header wraps to two lines~~ | **RESOLVED 2026-08-24, task `040240826`.** The nav collapsed into a burger below `md`, so the four links leave the row entirely. Minimum content width measured at **334px**, down from ~482px after `029240826` and ~562px before `028240826` — clears every real phone. `flex-wrap` removed so a future overflow is visible rather than silently absorbed; the wordmark truncates rather than overflowing below 334px. **Still not seen at a real mobile viewport** — the automation window would not resize below ~1400px, so the breakpoint was simulated | — |
...
| Arabic review — 2 menu strings | `menu_open` → `فتح القائمة`, `menu_close` → `إغلاق القائمة` (migration 0052, task `040240826`). **Written from the English by the assistant, not reviewed.** Seeded so the burger is not a nameless icon in Arabic; the migration's own comment marks them | Moataz |
| Arabic review — the whole career timeline | Seven job titles, five domains, four cities, four countries (migration 0056), plus `career_heading` → `المسار المهني` and `career_present` → `حتى الآن` (0054). Task `044240826`. **All written from the English by the assistant, none reviewed.** Live on `/ar/about`. One judgement to rule on: the job titles are TRANSLATED rather than kept Latin — `learn.md` records that Arabic here keeps `Governance`/`OTP`/`KYC` in Latin, and titles could join that list | Moataz |
| Employer name in the About prose | The timeline deliberately names no employer, but the existing `now` section still reads *"…at Mashreq, across the UAE and Egypt…"* — his own prose, predating the timeline. Found by the leak scan in `044240826`. Keep or remove is his call; flagged because it sits oddly beside a timeline built to omit it | Moataz |
